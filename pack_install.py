...
if len(args) == 0:
	try:
		...
		logging.error("Something wrong, not installed, check internet connection or package name")
	finally:
		sys.exit()	 
...
try:
	client.connect(hostname=HOST_IP, username=USERNAME, password=PASSWORD, port=int(PORT))
